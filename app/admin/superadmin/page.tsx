...
export default function SuperAdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<"restaurants" | "affiliates">("restaurants");

  const affiliates = restaurants.reduce((acc, r) => {
    if (r.affiliateVoucher) {
      const code = r.affiliateVoucher.toUpperCase().trim();
      if (!acc[code]) {
        acc[code] = { count: 0, activeCount: 0, restaurants: [] };
      }
      acc[code].count += 1;
      const isActive = r.subscriptionStatus === "active";
      if (isActive) acc[code].activeCount += 1;
      acc[code].restaurants.push(r.name);
    }
    return acc;
  }, {} as Record<string, { count: number, activeCount: number, restaurants: string[] }>);
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex gap-4 mb-6 border-b border-[var(--color-border)]">
          <button 
            ...
            className={`pb-3 px-2 font-bold text-sm border-b-2 transition-colors ${activeTab === "restaurants" ? "border-[var(--color-orange)] text-[var(--color-orange)]" : "border-transparent text-[var(--color-text-muted)] hover:text-[var(--color-text)]"}`}
          >
            Restaurantes ({restaurants.length})
          </button>
          <button 
            onClick={() => setActiveTab("affiliates")}
            className={`pb-3 px-2 font-bold text-sm border-b-2 transition-colors ${activeTab === "affiliates" ? "border-[var(--color-orange)] text-[var(--color-orange)]" : "border-transparent text-[var(--color-text-muted)] hover:text-[var(--color-text)]"}`}
          >
            Comerciais / Parceiros ({Object.keys(affiliates).length})
          </button>
        </div>
                    ...
                    <tr key={rest._id} className="border-b border-[var(--color-border)] hover:bg-[var(--color-surface-2)]">
                      <td className="p-4">
                        ...
                        <p className="text-xs text-[var(--color-text-muted)]">
                          ...
                          {rest.affiliateVoucher && <span className="ml-2 inline-block px-1.5 py-0.5 bg-[var(--color-surface)] border border-[var(--color-border)] rounded text-[10px] text-yellow-500">Voucher: {rest.affiliateVoucher}</span>}
                        </p>
                      </td>
                      <td className="p-4">
        ...
        {activeTab === "affiliates" && (
          <div className="glass-card overflow-x-auto">
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-[var(--color-border)]">
                  <th className="p-4 font-semibold text-sm">Voucher / Comercial</th>
                  <th className="p-4 font-semibold text-sm">Total Indicados</th>
                  <th className="p-4 font-semibold text-sm">Restaurantes Ativos (Pagantes)</th>
                  <th className="p-4 font-semibold text-sm">Comissão Estimada (R$ 75 / ativo)</th>
                </tr>
              </thead>
              <tbody>
                {Object.keys(affiliates).map((code) => {
                  const data = affiliates[code];
                  const commission = data.activeCount * 75;

                  return (
                    <tr key={code} className="border-b border-[var(--color-border)] hover:bg-[var(--color-surface-2)]">
                      <td className="p-4">
                        <span className="font-bold text-lg text-yellow-500">{code}</span>
                      </td>
                      <td className="p-4 font-medium">
                        {data.count}
                        <p className="text-[10px] text-[var(--color-text-muted)] mt-1 max-w-[200px] truncate" title={data.restaurants.join(", ")}>
                          {data.restaurants.join(", ")}
                        </p>
                      </td>
                      <td className="p-4">
                        <span className="px-3 py-1 text-xs font-bold rounded-full bg-blue-500/10 text-blue-500">
                          {data.activeCount} ativos
                        </span>
                      </td>
                      <td className="p-4">
                ...
                {Object.keys(affiliates).length === 0 && (
                  <tr>
                    <td colSpan={4} className="p-8 text-center text-[var(--color-text-muted)]">Nenhum voucher registrado ainda.</td>
                  </tr>
                )}
              </tbody>
            </table>
